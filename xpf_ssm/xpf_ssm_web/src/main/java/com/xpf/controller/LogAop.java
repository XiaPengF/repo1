...
public class LogAop {
    ...
    @Before("execution(* com.xpf.controller.*.*(..))")
    public void doBefore(JoinPoint jp) throws NoSuchMethodException, SecurityException {
        ...
        String methodName = jp.getSignature().getName();// 获取访问的方法:获取访问的方法的名称
    ...
    @After("execution(* com.xpf.controller.*.*(..))")
        ...
        if (executionClass != SysLogController.class) {
            ...
            if (classAnnotation != null) {
                ...
                if (methodAnnotation != null) {
                    ...
                    sysLog.setExecutionTime(executionTime);// 将sysLog对象属性封装
                    sysLog.setUrl(url);// 将sysLog对象属性封装

                    String ip = request.getRemoteAddr();// 获取ip
                    sysLog.setIp(ip);// 将sysLog对象属性封装
                    ...
                    sysLog.setUsername(username);// 将sysLog对象属性封装

                    sysLog.setMethod("[类名]" + executionClass.getName() + "[方法名]" + executionMethod.getName());

                    sysLog.setVisitTime(startTime);// 将sysLog对象属性封装
